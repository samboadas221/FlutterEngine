...
// OLD CHATGPT CUSTOM BUTTON WORKING
...
class _CustomButtonWidgetState extends State<_CustomButtonWidget> {
  ...
  Future<void> _playClickSound(String assetPath, SoundManager audioPlayer) async {
    try {
      audioPlayer.playEffect(assetPath);
    } catch (e) {
      // Nothing
    }
  }
...
class _CustomButtonWidgetState extends State<_CustomButtonWidget> {
  ...
  Future<void> _playClickSound(String assetPath) async {
    try {
      await SoundManager().playAsset(assetPath); // Usa la instancia singleton estática
    } catch (e) {
      // Nothing
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onTapUp: (_) async {
        setState(() => _isPressed = false);
        if (widget.clickAudioAsset != null) {
          await _playClickSound(widget.clickAudioAsset!);
        }
        widget.onPressed?.call();
      },
      onTapCancel: () => setState(() => _isPressed = false),
